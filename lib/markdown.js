...
renderer.urltransform = function (url) {
  ...
  else if (url.indexOf('http') !== 0 && url.indexOf('ssb') !== 0) {
    return false;
  }
  return url
}
...
renderer.link = function(href, title, text) {
  ...
  // make a popup if http/s or ssb
  if (href && (href.indexOf('http') === 0 || href.indexOf('ssb') === 0))
    out += ' target="_blank"'
...
/*renderer.image  = function (href, title, text) {
  ...
  if (ssbref.isLink(href)) {
    ...
    var out = '<img src="ssb:' + href + '" alt="' + text + '"'
    if (title) {
      out += ' title="' + title + '"'
    }
    out += '>'
