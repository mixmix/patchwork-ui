...
module.exports = function (app) {
  ...
  app.ui.dropdown = function (el, options, cb) {
    var dropdown = h('.dropdown',
      { onmouseleave: die },
      options.map(function (o) {
        if (o.separator)
          return h('hr')
        return h('a', { href: '#', onclick: onselect, 'data-value': o.value }, o.label)
      })
    )

    var rect = el.getClientRects()[0]
    dropdown.style.top = (rect.bottom + document.body.scrollTop + 10) + 'px'
    dropdown.style.left = (rect.left + document.body.scrollLeft - 20) + 'px'
