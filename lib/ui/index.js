...
module.exports.refreshPage = function (e, cb) {
  ...
  app.fetchLatestState(function() {
    ...
    // h.cleanup() :TODO: replace with smarter system
...
module.exports.setPage = function (name, page, opts) {
  ...
  // render page
  var pageEl = document.querySelector('#page .wm-content')
  pageEl.innerHTML = ''
  pageEl.appendChild(h('.'+name+'-page', page))
  setNavAddress()
...
module.exports.setNavAddress = function (location) {
  ...
  document.body.querySelector('#page .wm-nav input').value = location
}
  ...
  function doit() {
    ...
    if (enabled === false) {
      ...
      uhohTimer = setTimeout(function () {
        setStatus('Hmm, this seems to be taking a while...')
      }, 5e3)
      tooLongTimer = setTimeout(function () {
        setStatus('I think something broke :(. Please restart Patchwork and let us know if this keeps happening!')
      }, 20e3)
    }
  }
