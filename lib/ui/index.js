...
module.exports.refreshPage = function (e, cb) {
  ...
  app.fetchLatestState(function() {
    ...
    // render the new page
    var page = app.pages[app.page.id]
    if (!page)
...
module.exports.setPage = function (name, page, opts) {
  ...
  pageEl.innerHTML = ''
  if (app.page.id != 'home') {
    pageEl.appendChild(h('.page-info',
      h('a', { href: '#', onclick: function (e) { e.preventDefault(); window.history.back() } }, h('span', { innerHTML: '&larr;' }), ' back'),
      ' ', app.page.param
    ))
  }
