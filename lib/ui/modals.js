...
module.exports = function (phoenix) {
  ...
  phoenix.ui.inviteModal = function (e) {
    ...
    function onsubmit (code) {
      ...
      if (inviteRegex.test(code)) {
        ...
        phoenix.ssb.invite.accept(code, addMeNext)
      }
      else
      ...
      function addMeNext (err) {
        ...
        modal.close()
        phoenix.ui.homeMode.live = true
        phoenix.ui.homeMode.view = 'party'
  ...
  phoenix.ui.flagModal = function (userId) {

    // render

    var form = com.flagForm(phoenix, userId, { onsubmit: onsubmit })
    var modal = phoenix.ui.modal(form)
