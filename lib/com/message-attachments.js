...
module.exports = function (msg) {
  ...
  mlib.indexLinks(msg.value.content, { ext: true }, function (link, rel) {
    ...
    if (isImage(link))
      els.push(h('a', { href: url }, h('.image', { 'data-bg': 'pwblob:'+encodeURI(link.link), style: 'background-image: url(pwblob:'+encodeURI(link.link)+'?fallback=img)' })))
    else
