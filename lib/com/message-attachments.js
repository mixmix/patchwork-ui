...
module.exports = function (msg) {
  ...
  mlib.indexLinks(msg.value.content, { ext: true }, function (link, rel) {
    ...
    var url = '#/webview/'+encodeURI(link.ext)
    if (isImage(link))
      els.push(h('a', { href: url }, h('.image', { 'data-bg': 'blob:'+encodeURI(link.ext), style: 'background-image: url(blob:'+encodeURI(link.ext)+'?fallback=img)' })))
    else
