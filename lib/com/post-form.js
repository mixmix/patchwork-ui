...
module.exports = function (parent, opts) {
  ...
  var placeholder = (opts && opts.placeholder) ?
    opts.placeholder : 
    (!parent ? 'Share a message with the world...' : 'Reply to this thread...')
  ...
  function enable () {
    ...
    delete textarea.style.borderRadius
  }
  ...
  function post (e) {
    ...
    // abort if the parent wasnt decryptable
    ...
    mentionslib.extract(text, function (err, mentions) {
      ...
      try {
        if (Array.isArray(parent.value.content.recps)) {
          recps = mlib.links(parent.value.content.recps)
      ...
      // post
      var post = schemas.post(text, parent && parent.key, mentions, recps)
      if (recps)
