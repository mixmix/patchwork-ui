...
exports.pagenav = function (app) {
  ...
  var dropdown = h('.dropdown-inner',
    h('a', { href: '#/sync' }, 'Network Sync'),
    h('a', { href: '#/feed' }, 'Under the Hood')
  )
  function ondropdown (e) {
    var cl = e.target.classList
    if (cl.contains('dropdown') || cl.contains('glyphicon-triangle-bottom')) {
      e.preventDefault()
  ...
  return h('.page-nav-inner',
    ...
    h('.divider'),
    addressInput,
    h('a.dropdown', { href: '#', onclick: ondropdown, onmouseleave: ondropup },
    ...
    h('a.profile', { href: '#/profile/'+app.user.id }, h('img', { src: profilePicUrl(app, app.user.id), title: 'Your Profile' }))
  )
...
exports.sidehelp = function (app, opts) {
  ...
  return h('ul.list-unstyled.sidehelp',
    h('li',
      ...
      h('label', h('input', { type: 'checkbox', onclick: onlivetoggle, checked: app.ui.homeMode.live }), ' Live mode')),
    h('li', h('hr')),
    // h('li', h('button.btn.btn-link', { onclick: app.ui.showUserId }, 'Get your id')),
    h('li', h('a.btn.btn-3d', { href: '#/address-book' }, com.icon('search'), ' Search People')),
    h('li', h('a.btn.btn-3d', { onclick: app.ui.followPrompt }, com.icon('send'), ' Invite a Friend')),
