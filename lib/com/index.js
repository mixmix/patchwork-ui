...
function findpeer (app, id) {
  for (var i=0; i < app.peers.length; i++) {
    if (app.peers[i].key == id) {
      return app.peers[i]
    }
  }
}
...
var pubstats =
exports.pubstats = function (app, id) {
  var ls = (app.ui.homeMode.live) ? ' Livestream' : ''

  if (id == 'party')
    return h('.pubstats', h('p', h('strong', 'Party'+ls)), h('p', 'Everybody in your network'))
  if (id == 'friends')
    return h('.pubstats', h('p', h('strong', 'Friends'+ls)), h('p', 'Just the people you follow'))

  var peer = findpeer(app, id)
  if (!peer)
    return

  var members = social.followeds(app, id)
  var ismember = members.indexOf(app.user.id) !== -1
  var name = peer.host
  name = (name) ? (name.slice(0,1).toUpperCase() + name.slice(1)) : id
  var lastsync = h('p.text-danger', 'Has not synced recently')
  if (peer.time) {
    if (peer.time.connect > peer.time.attempt)
      lastsync = h('p.text-success', 'Synced '+ago(peer.time.connect, true))
    else if (peer.time.attempt) {
      lastsync = h('p.text-danger', 'Tried & failed to sync '+ago(peer.time.attempt, true))
    }
  }
  return h('.pubstats',
    h('p.name', h('a', { href: '#/profile/'+id }, name, ls)),
    h('p', members.length + ' members', (ismember ? h('small', ' including you!') : '')),
    lastsync
  )
}
...
exports.sidenav = function (app, opts) {
  var ps = pubstats(app, app.ui.homeMode.view)
  function onviewclick (e) { 
    ...
    var options = [
      { value: 'party',   label: [h('strong', 'Party'), h('br'), 'Everybody in your network'] },
      { value: 'friends', label: [h('strong', 'Friends'), h('br'), 'Just the people you follow'] }
    ].concat(app.peers.map(function (peer) {
      var members = social.followeds(app, peer.key)
      return { value: peer.key, label: [h('strong', capitalized(peer.host || peer.key)), h('br'), members.length, ' members'] }
    }))

    app.ui.dropdown(ps, options, { offsetX: 15, width: 300 }, function (value) {
      if (!value) return
      app.ui.homeMode.view = value
      app.refreshPage()
    })
  }
  ...
  function onfiltertoggle (filter) {
    return function () {
      var hide = !app.ui.filters[filter]
      app.ui.filters[filter] = hide
      if (!hide)
        document.querySelector('.message-feed').classList.add('show-'+filter)
      else
        document.querySelector('.message-feed').classList.remove('show-'+filter)
    }
  }

  return h('ul.list-unstyled.sidenav',
    // h('li.viewselect', 
    //   h('a.btn.btn-3d', { href: '#', onclick: onviewclick }, icon('chevron-down'), ' ', capitalized(viewLabel)),
    //   h('a.toggle'+(app.ui.homeMode.live?'.pressed':''), { href: '#', onclick: onlivetoggle, title: 'Livestream' }, icon('flash'))
    // ),
    h('div', { onclick: onviewclick }, ps),
    h('li', h('hr')),
    ...
    (!opts || !opts.noMore) ? h('li', h('span', {style:'display: inline-block; padding: 0 12px 6px'}, a('#/help', [com.icon('question-sign'), 'Help']))) : ''
    // h('li', h('hr')),
    // h('li.filters',
    //   'Filters: ',
    //   h('label', h('input', { type: 'checkbox', onclick: onfiltertoggle('nsfw'), checked: app.ui.filters.nsfw }), ' NSFW'),
    //   ' ',
    //   h('label', h('input', { type: 'checkbox', onclick: onfiltertoggle('spam'), checked: app.ui.filters.spam }), ' Spam'),
    //   ' ',
    //   h('label', h('input', { type: 'checkbox', onclick: onfiltertoggle('abuse'), checked: app.ui.filters.abuse }), ' Abuse')
    // )
  )
}
