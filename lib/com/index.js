...
exports.sidenav = function (app, opts) {
  function onviewclick (e) { 
    ...
    var options = [
      ...
      return { value: peer.key, label: app.users.names[peer.key] || peer.host || peer.key }
    }))

    app.ui.dropdown(e.target, options, { offsetX: 15 }, function (value) {
      if (!value) return
  ...
  var peer = findpeer(app, app.ui.homeMode.view)
  var viewLabel = peer && peer.host || app.ui.homeMode.view
  return h('ul.list-unstyled.sidenav',
    ...
    // pubviewHexagrid(app),
    pubstats(app, app.ui.homeMode.view),
    // h('li', h('label.livemode', h('input', { type: 'checkbox', onclick: onlivetoggle, checked: app.ui.homeMode.live }), ' Livestream')),
    h('li', h('hr')),
