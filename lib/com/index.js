...
exports.user = function (app, id, opts) {
  ...
  return [l(id, userName(app, id)), followIcon]
}
  ...
  if (Object.keys(app.user.profile.assignedTo).length == 0) {
    return h('.welcome-help',
      ...
      h('.big-btn', { onclick: function () { alert('Todo!') } },
        ...
        h('p', 'Are you the first of your friends on Patchwork? You trend-setter! Click here to get an invite from the devs.')
      ),
...
exports.sidenav = function (app, opts) {
  function onparty ()   { app.ui.homeMode.view = 'party' }
  function onfriends () { app.ui.homeMode.view = 'friends' }
  ...
  return h('ul.list-unstyled.sidenav',
    h('li',
      h('.btn-group',
        h('a.btn.btn-3d'+(app.ui.homeMode.view == 'party'  ?'.pressed':''), { href: '#/', onclick: onparty },   'Party'),
        h('a.btn.btn-3d'+(app.ui.homeMode.view == 'friends'?'.pressed':''), { href: '#/', onclick: onfriends }, 'Friends')),
      h('label.livemode', h('input', { type: 'checkbox', onclick: onlivetoggle, checked: app.ui.homeMode.live }), ' Livestream')),
    h('li.filters',
      'Filters: ',
    ...
    h('li', h('a.btn.btn-3d', { onclick: app.ui.inviteModal }, com.icon('ok'), ' Accept an Invite')),
    h('li', h('span', {style:'display: inline-block; padding: 4px 13px 2px'}, h('a', { href: 'https://github.com/ssbc', target: '_blank' }, [com.icon('bullhorn'), 'Send Feedback']))),
    (!opts || !opts.noMore) ? h('li', h('span', {style:'display: inline-block; padding: 0 13px 6px'}, a('#/help', [com.icon('question-sign'), 'Help']))) : ''
  )
}
