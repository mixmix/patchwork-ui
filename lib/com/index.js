...
exports.pagenav = function () {
  ...
  function onmenuclick (e) {
    ...
    if (!stats.membersof || !stats.active)
      syncWarningIcon = icon('warning-sign.text-danger')
  ...
  function oninboxclick (e) {
    ...
    var el = this
    pull(
      app.ssb.phoenix.createInboxStream({ reverse: true, limit: 10 }),
      pull.collect(function (err, items) {
        items = items || []
        var opts = { menuitem: true }
        items = items.map(function (msg) { return exports.messageOneline(msg, opts) }).filter(Boolean)
        ui.dropdown(el, 
          [h('a.item', { href: '#/inbox', style: 'padding: 10px' }, 'Open Inbox')].concat(items),
          { right: true, offsetY: 5, width: 300, cls: '.nopad' }
        )
      })
    )
  }
  function onstarsclick (e) {
    ...
    var el = this
    pull(
      app.ssb.phoenix.createVoteStream({ reverse: true, limit: 10 }),
      pull.collect(function (err, items) {
        items = items || []
        var opts = { menuitem: true }
        var menuitems = items.map(function (msg) { return exports.messageOneline(msg, opts) }).filter(Boolean)
        ui.dropdown(el, 
          [h('a.item', { href: '#/stars', style: 'padding: 10px' }, 'Open Stars')].concat(menuitems),
          { right: true, offsetY: 5, width: 300, cls: '.nopad' }
        )
        if (items.length)
          app.ssb.phoenix.markRead(items.map(function (item) { return item.key }))
      })
    )
  }
  ...
  return h('.page-nav-inner',
    ...
    addressInput,
    h('.divider'),
    h('a.action', { href: '#', onclick: function (e) { e.preventDefault(); subwindows.pm() } }, 'Secret Message'),
    h('a.stat', { href: '#', onclick: oninboxclick }, icon('envelope'), ' ', app.observ.indexCounts.inbox, inboxUnread),
    h('a.stat', { href: '#', onclick: onstarsclick }, icon('star'), ' ', app.observ.indexCounts.votes, votesUnread),
    h('a.stat', { href: '#/friends' }, icon('user'), ' ', app.observ.indexCounts.follows, followsUnread),
    h('a.profile', { href: '#', onclick: onmenuclick }, h('img', { src: profilePicUrl(app.user.id) }))
  )
...
exports.helpTitle = require('./help').helpTitle