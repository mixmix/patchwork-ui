    ...
    var s = ({
      post: function () { 
        ...
        if (mlib.link(c.repliesTo, 'msg'))
          return com.a('#/msg/'+mlib.link(c.repliesTo).msg, t(c.text))
      ...
      vote: function () {
        ...
        if (c.vote == 1)
          link = h('a', { href: '#/msg/'+mlib.link(c.voteTopic).msg }, com.icon('star'), ' ', desc)
        else if (c.vote <= 0)
          link = h('a', { href: '#/msg/'+mlib.link(c.voteTopic).msg }, com.icon('erase'), ' ', desc)
        appendMsgSummary(desc, mlib.link(c.voteTopic).msg, t)
        return link
      },
      flag: function () {
...
module.exports = function (msg, opts) {
  ...
  if (opts && opts.menuitem) {
    ...
    var href = '#/msg/'
    if (mlib.link(c.repliesTo, 'msg'))
      href += mlib.link(c.repliesTo).msg
    else if (mlib.link(c.voteTopic, 'msg'))
      href += mlib.link(c.voteTopic).msg
    else
      href += msg.key
