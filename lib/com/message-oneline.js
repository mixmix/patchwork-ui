    ...
    var s = ({
      post: function () { 
        ...
        if (mlib.link(c.repliesTo, 'msg'))
          return com.a('#/msg/'+mlib.link(c.repliesTo).link, t(c.text))
      ...
      vote: function () {
        if (!mlib.link(c.voteTopic, 'msg'))
          return
        var link, desc = h('span', 'this message')
        if (c.vote == 1)
          link = h('a', { href: '#/msg/'+mlib.link(c.voteTopic).link }, com.icon('star'), ' ', desc)
        else if (c.vote <= 0)
          link = h('a', { href: '#/msg/'+mlib.link(c.voteTopic).link }, com.icon('erase'), ' ', desc)
        appendMsgSummary(desc, mlib.link(c.voteTopic).link, t)
        return link
      },
      flag: function () {
        if (!mlib.link(c.flagTopic, 'msg'))
          return
        if (c.flag)
          return com.a('#/msg/'+mlib.link(c.flagTopic).link, h('span.text-danger', com.icon('flag'), ' Flagged your post ', h('span.label.label-danger', c.flag)))
        else
          return com.a('#/msg/'+mlib.link(c.flagTopic).link, h('span.text-danger', com.icon('erase'), ' Unflagged your post'))
      }
    })[c.type]()
    if (!s || s.length == 0)
      s = false
...
module.exports = function (msg, opts) {
  ...
  if (typeof msg.value.content == 'string') {
    ...
    content = h('div')
    app.ssb.crypto.unbox(msg.value.content, function (err, decrypted) {
      if (decrypted) {
  ...
  if (opts && opts.menuitem) {
    ...
    var href = '#/msg/'
    if (mlib.link(c.repliesTo, 'msg'))
      href += mlib.link(c.repliesTo).link
