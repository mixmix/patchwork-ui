...
exports.helpBody = function (item) {
  if (item == 'howto-pubs') {
    return h('div', { style: 'padding: 20px' },
      h('p',
        ...
        'A public node is a usually a cloud-computer, but it could be any device with a public address. '
      ),
      h('br'),
      h('p',
        h('strong', 'During the closed beta, you have to know somebody that runs a pub. '),
        '(This is because we\'re a super-exclusive new network, not because we\'re a pain-in-the-ass beta with a bad UX.) ',
        'Ask the pub owner for an invite code, then ', h('a', { href: '#', onclick: modals.invite }, 'click here to join their pub'), '.'
      ),
      h('br'),
      h('p',
      ...
      h('br'),
      h('p',
        h('strong', 'If you\'re not sure whether you\'re a neckbeard,'), ' administer this simple test: ',
        'First, put your left hand on your neck. ',
        'Then, gently rub up and down. ',
        'If you do not feel any hair, then you are not a neckbeard, and you should ', h('em', 'not'), ' attempt to set up a public node.'
      )
    )
  }
  if (item == 'howto-find-ppl') {
    return h('div', { style: 'padding: 20px' },
      ...
      h('p', 
        'Click the ', com.icon('user'), ' icon on the top right to see ', h('a', { href: '#/friends' }, 'your friends page'), '. ',
  ...
  if (item == 'howto-posts') {
    return h('div', { style: 'padding: 20px' },
      h('p',
        h('strong', 'Go to the ', com.a('#/', 'home page')), ', click on the input box at the top and start typing. ',
  ...
  if (item == 'howto-webcam') {
    return h('div', { style: 'padding: 20px' },
      h('p',
        h('strong', 'Go to the ', com.a('#/', 'home page')), ', then mouse over the ', com.icon('comment'), ' icon next to the input box at the top. ',
  ...
  if (item == 'howto-secret-messages') {
    return h('div', { style: 'padding: 20px' },
      h('p', h('strong', 'Method 1: Profile Page')),
      h('p',
        'Open the profile page of the user you want to message. ',
        'Then, click "Secret Message." ',
        'Your friend will automatically be added to the recipients.'
      ),
      h('br'),
      h('p', h('strong', 'Method 2: Inbox Page')),
      h('p',
        'Open your ', com.a('#/inbox', 'inbox page'), ' and click "Secret Message." ',
        'Then, manually add your recipients.'
      )
    )
  }
}
    ...
    h('.message-inner',
      ...
      h('.message-body',
        h('.markdown',
          ...
          h('p', 
            'Patchwork is an independent network of computers. ',
            'The software is Free and Open-source, and the data is stored on your computer.'
          ),
          h('p', h('img.emoji', { src: './img/emoji/facepunch.png', height: 20, width: 20}), ' Take that, corporate overlords!')
        )
      )
    ),
    h('.message-comments',
...
exports.side = function () {
  ...
  function help (topic, text) {
    return [
      h('a.text-muted', { href: '#', onclick: onhelp(topic), title: text }, com.icon('question-sign'), ' ', text),
      h('br')
    ]
  }
  ...
  function link (url, text) {
    return [
      h('a.text-muted', { href: url, title: text }, com.icon('arrow-right'), ' ', text),
      h('br')
    ]
  }
  ...
  return h('div',
    h('p',
      h('strong.text-muted', 'Help Topics:'), h('br'),
    ...
    h('p',
      h('strong.text-muted', 'Quick Links:'), h('br'),
      link('#/profile/'+app.user.id, 'View my profile'),
      link('#/setup', 'Change my name or profile pic'),
      link('#/inbox', 'Check my inbox messages'),
      link('#/friends', 'View my friends'),
      h('a.text-muted.noicon', { href: 'https://github.com/ssbc/patchwork/issues/new', target: '_blank', title: 'File a suggestion or issue' }, com.icon('bullhorn'), ' File an Issue')
    )
  )
}