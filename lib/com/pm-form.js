...
module.exports = function (app, parent, opts) {
  ...
  function onSelectRecipient (e) {
    ...
    if (recipients.length >= 7)  {
      app.ui.notice('warning', 'Cannot add @'+recpInput.value+' - You have reached the limit of 7 recipients on a Secret Message.')
      recpInput.value = ''
      return
    }
    ...
    uploadFiles(function (err, extLinks) {
      ...
      mentionslib.extract(app, text, function (err, mentions) {
        ...
        var recps = recipients.map(function (id) {
          return { feed: id, name: com.userName(app, id) }
        })
        ...
        if (parent)          mail.repliesTo = { msg: parent.key }
