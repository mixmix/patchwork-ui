...
exports.pagenav = function () {
  ...
  function onmenuclick (e) {
    ...
    ui.dropdown(this, [
      ...
      h('hr'),
      h('a.item', { href: '#/drive' }, 'Your Drive ', h('span', { style: 'margin-left: 5px' }, com.icon('hdd'))),
      h('hr'),
  ...
  function oninboxclick (e) {
    ...
    pull(
      ...
      pull.collect(function (err, items) {
        ...
        items = items.map(function (msg) { return com.messageOneline(msg, opts) }).filter(Boolean)
        ui.dropdown(el, 
  ...
  function onstarsclick (e) {
    ...
    pull(
      ...
      pull.collect(function (err, items) {
        items = items || []
        var opts = { menuitem: true }
        var menuitems = items.map(function (msg) { return com.messageOneline(msg, opts) }).filter(Boolean)
        ui.dropdown(el, 
  ...
  return h('.page-nav-inner',
    ...
    h('a.stat', { href: '#', onclick: oninboxclick }, com.icon('envelope'), ' ', app.observ.indexCounts.inbox, inboxUnread),
    h('a.stat', { href: '#', onclick: onstarsclick }, com.icon('star'), ' ', app.observ.indexCounts.votes, votesUnread),
