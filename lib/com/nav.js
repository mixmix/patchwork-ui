...
function addressbar () {
  ...
  if (location == 'profile' || location == 'webview')
...
exports.pagenav = function () {
  ...
  function onwebview (segment) {
    return function (e) {
      e.preventDefault()
      Array.prototype.forEach.call(document.querySelectorAll('.page-nav-inner .stat:not(.'+segment+')'), function (el) { el.classList.remove('selected') })
      document.querySelector('.page-nav-inner .stat.'+segment).classList.toggle('selected') 
      Array.prototype.forEach.call(document.querySelectorAll('.webview-right:not(.'+segment+')'), function (el) { el.classList.add('hide') })
      document.querySelector('.webview-right.'+segment).classList.toggle('hide')
    }
  }
  ...
  return h('.page-nav-inner',
    ...
    addressbar(),
    (app.page.id == 'webview') ? [
      (isref.isHash(app.page.param)) ? h('a.stat.comments', { href: '#', onclick: onwebview('comments') }, com.icon('comment')) : '',
      h('a.stat.editor', { href: '#', onclick: onwebview('editor') }, com.icon('edit')),
      h('.divider')
    ] : '',
