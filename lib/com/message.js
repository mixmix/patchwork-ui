...
var messageContent = require('./message-content')
  ...
  var msgEl = h('.message'+(!msg.plaintext?'.secret':'')+((opts && opts.fullview)?'.fullview':'.smallview'),
    ...
    h('.message-inner',
      h('ul.message-header.list-inline',
        ...
        !msg.plaintext ? h('li', 'encrypted') : '',
        h('li.pull-right', h('a', { href: '#', onclick: onflag(msg), title: 'Flag this post', innerHTML: '&spades;' })),
        h('li.favorite.pull-right',
          h('span.users'),
          h('a', { href: '#', onclick: onfavorite(msg), title: 'Favorite this post', innerHTML: '&starf;' })
        )
      ),
      h('.message-body', (typeof msg.value.content != 'string') ? messageContent(msg) : ''),
  ...
  if (!msg.plaintext) {
    app.ssb.private.unbox(msg.value.content, function (err, decrypted) {
      if (decrypted) {
        ...
        body.innerHTML = ''
        body.appendChild(messageContent(msg))
      }
    })
  }
...
module.exports.fetchState = function (msg, opts) {
  ...
  app.ssb.relatedMessages({ id: msg.key, count: true }, function (err, thread) {
    if (!thread || !thread.related) {
      if (opts && opts.fullview)
        msg.el.appendChild(postForm(msg, msg, { onpost: onpostreply(msg, opts), noheader: true }))
    ...
    u.decryptThread(thread, function () {
      ...
      if (opts && opts.fullview)
        msg.el.appendChild(postForm(thread, comments[comments.length - 1] || thread, { onpost: onpostreply(msg, opts), noheader: true }))
  ...
  var el = h('.message',
    ...
    h('.message-inner',
      h('ul.message-header.list-inline',
        ...
        (msg.plaintext === false) ? h('li', 'encrypted') : '',
        h('li.pull-right', h('a', { href: '#', onclick: onflag(msg), title: 'Flag this post', innerHTML: '&spades;' })),
        h('li.favorite.pull-right',
          h('span.users'),
          h('a', { href: '#', onclick: onfavorite(msg), title: 'Favorite this post', innerHTML: '&starf;' })
