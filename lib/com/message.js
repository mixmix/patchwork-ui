    ...
    var s = ({
      ...
      mail: function () {
        ...
        var recps = c.recps.map(function (r, n) { 
          var user = com.user(app, r.feed, { thin: true })
  ...
  if (typeof msg.value.content == 'string') {
    ...
    msgEl = h('.message',
      ...
      h('ul.message-header.list-inline',
        ...
        h('li.pull-right', com.icon('lock'), ' Encrypted thread')
      ),
    ...
    app.ssb.unbox(msg.value.content, function (err, decrypted) {
      if (decrypted) {
        ...
        body.innerHTML = ''
        body.appendChild(getContent(app, msg, opts))
    ...
    fetchStateEncrypted(app, msgEl, msg.key, opts)
  } else {
    // plaintext
    ...
    msgEl = h('.message',
      ...
      h('.message-body', getContent(app, msg, opts)),
      com.messageAttachments(app, msg),
      msgComments
    )
    fetchStatePlaintext(app, msgEl, msg.key, opts)
  }
  
  // add reply form for fullview  
  if (opts && opts.fullview) 
    msgEl.appendChild(com.postForm(app, msg, { rows: 5, placeholder: 'Write your reply', noheader: true, onpost: onfullviewpost }))
  ...
  function onreply (e) {
    ...
    function onpostreply (comment) {
      ...
      var cdiv = h('.comment',
        ...
        h('.comment-inner', getContent(app, comment)))
      ...
      app.ssb.publish({ type: 'flag', flagTopic: { msg: msg.key }, flag: value }, function (err, flagmsg) {
        if (err) {
          ...
          msgEl.querySelector('.message-comments').appendChild(h('.comment',
            ...
            h('.comment-inner', getContent(app, flagmsg))
          ))
        }
      })
    })
...
module.exports.fetchStatePlaintext = function (app, el, mid, opts) {
...
module.exports.fetchStateEncrypted = function (app, el, mid, opts) {
  mid = mid || el.dataset.msg
  if (!mid) return
  pull(
  ...
  function render (err, comments) {
    ...
    comments.forEach(function (c) {
      var cdiv = h('.comment',
        com.userImg(app, c.value.author),
        h('.comment-inner', 
          c.plaintext ? h('em.text-danger.pull-right', 'Warning: This comment was not encrypted!') : '',
          getContent(app, c)
        )
      )
      el.querySelector('.message-comments').appendChild(cdiv)
