  ...
  try {
    var s = ({
      ...
      flag: function () { 
        ...
        if (c.flag && app.user.id == msg.value.author) {
          ...
          function onunflag (e) {
            ...
            // publish unflag
            app.ssb.publish({ type: 'flag', flagTopic: c.flagTopic, undoesFlag: msg.key, flag: false }, function (err, flagmsg) {
              if (err) {
                console.error(err)
      ...
      vote: function () {
        ...
        var topic = mlib.link(c.voteTopic)
        if (!c.voteTopic)
          return false
...
module.exports = function (msg, opts) {
  ...
  if (typeof msg.value.content == 'string') {
    ...
    msgEl = h('.message',
      ...
      h('ul.message-header.list-inline',
        h('li', com.user(msg.value.author)),
        h('li', com.a('#/msg/'+msg.key, u.prettydate(new Date(msg.value.timestamp), true))),
        h('li', h('a', { href: '#', onclick: onflag, title: 'Flag this post' }, com.icon('flag'))),
  ...
  function onflag (e) {
    ...
    ui.dropdown(e.target, [
      ...
      if (!value) return
      // publish flag
      app.ssb.publish({ type: 'flag', flagTopic: msg.key, flag: value }, function (err, flagmsg) {
        if (err) {
          console.error(err)
          swal('Error While Publishing', err.message, 'error')
        } else {
  ...
  function onfavorite (e) {
    ...
    app.ssb.publish({ type: 'vote', voteTopic: msg.key, vote: newvote }, function (err) {
      voting = false
      if (err) {
...
function isFlagUndone (r) {
  if (r.related) {
    return r.related.filter(function (msg) {
      var c = msg.value.content
      return (mlib.link(c.undoesFlag) && mlib.link(c.undoesFlag).link == r.key)
    }).length > 0
  }
  return false
}
