...
function getSummary (app, msg) {
  ...
  try {
    var s = ({
      ...
      flag: function () { 
        if (!c.flag)
          return h('p.text-danger', com.icon('erase'), ' Unflagged')
        if (typeof c.flag == 'string')
          return h('p.text-danger', com.icon('flag'), ' ', h('span.label.label-danger', c.flag))
        return h('p.text-danger', com.icon('flag'), ' Flagged')
      },
      fact: function () { 
        if (!c.text) return
...
module.exports = function (app, msg, opts) {
  ...
  function onflag (e) {
    ...
    app.ui.dropdown(e.target, [
      ...
      if (!value) return
      app.ssb.publish({ type: 'flag', flagTopic: { msg: msg.key }, flag: value }, function (err) {
        if (err) {
          console.error(err)
          swal('Error While Publishing', err.message, 'error')
        } else {
          fetchRowState(app, msgSummary, msg.key, opts) // refresh state
        }
      })
    })
  }
...
module.exports.setRowState = function (app, el, thread, opts) {
  ...
  // handle flags
  el.classList.remove('flagged-nsfw', 'flagged-spam', 'flagged-')
