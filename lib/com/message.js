...
module.exports = function (msg, opts) {
  ...
  var msgEl = h('.message'+(isEncrypted?'.secret':''),
    com.userImg(msg.value.author),
    (isEncrypted?h('.secret-label',com.icon('lock'), ' Secret Thread'):''),
    h('.message-inner',
      h('ul.message-header.list-inline',
        ...
        isEncrypted ? h('li', com.icon('lock')) : '',
...
module.exports.fetchState = function (msg, opts) {
  app.ssb.relatedMessages({ id: msg.key, count: true }, function (err, thread) {
    if (!thread || !thread.related) {
      msg.el.appendChild(com.composer(msg, msg, { onpost: onpostreply(msg) }))
      return
    }
    ...
    u.decryptThread(thread, function () {
      ...
      msg.el.appendChild(com.composer(thread, comments[comments.length - 1] || thread, { onpost: onpostreply(msg) }))
      ...
      if (opts && opts.fullview)
        ...
        msg.el.appendChild(
          h('a.load-comments', { href: '#/msg/'+thread.key }, 'Show ' + comments.length + ' comment' + (comments.length!=1?'s':'')),
          msg.el.querySelector('.message-comments')
        )
      }
