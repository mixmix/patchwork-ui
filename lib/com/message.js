...
module.exports = function (msg, opts) {
  ...
  var msgEl = h('.message'+(!msg.plaintext?'.secret':'')+((opts && opts.fullview)?'.fullview':'.smallview'),
    ...
    (!msg.plaintext?h('.secret-label',com.icon('lock'), ' Secret Thread'):''),
    h('.message-inner',
      h('ul.message-header.list-inline',
        ...
        (!(opts && opts.fullview)) ?
          h('li', com.a('#/msg/'+msg.key, h('small.comment-count-digits', '0 replies'))) :
          '',
...
function onopen (msg) {
  return function (e) {
    ...
    var el = require('../ui/subwindows').subwindow(com.message(msg, { fullview: true, live: true }), 'msg')
    el.querySelector('.subwindow-body').style.background = '#eee'
  }
}
