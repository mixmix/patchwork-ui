...
module.exports = function (msg, opts) {
  ...
  if (opts && opts.fullview) 
    msgEl.appendChild(com.composer(msg, { onpost: onfullviewpost }))
  ...
  function onreply (e) {
    ...
    function onpostreply (comment) {
      ...
      msgEl.querySelector('.message-comments').appendChild(renderComment(comment))
    }

    if (!msgEl.querySelector('.reply-form')) {
      replyForm = com.composer(msg, { onpost: onpostreply })
    ...
    ui.dropdown(e.target, [
      ...
      app.ssb.publish(schemas.flag(msg.key, value), function (err, flagmsg) {
        if (err) {
          ...
          // render new flag
          msgEl.querySelector('.message-comments').appendChild(renderComment(flagmsg))
        }
      })
    })
  }
...
module.exports.fetchStateEncrypted = function (el, mid, opts) {
  ...
  function render (err, comments) {
    ...
    comments.forEach(function (c) {
      el.querySelector('.message-comments').appendChild(renderComment(c))
    })
