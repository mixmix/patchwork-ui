...
module.exports.table = function (obj, path) {
  ...
  for (var k in obj) {
    if (obj[k] && typeof obj[k] == 'object') {
      ...
      var name = encodeURIComponent(u.getExtLinkName(obj))
      var qs = (name) ? '?name='+name : ''
      els.push(row(k, com.a('blob:'+obj.ext+qs, obj.name || obj.ext, { target: '_blank' })))
    }
    else if (k == 'feed')
      els.push(row(k, com.user(obj.feed)))
    else
