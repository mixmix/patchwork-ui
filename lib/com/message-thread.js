...
module.exports = function (thread, opts) {

  // markup
  
  opts && opts.onrender && opts.onrender(thread)
  return h('.message-thread', 
    h('.message-thread-top', com.message(thread, opts)), 
    replies(thread, opts))
  ...
  function onreply (e) {
    ...
    if (!msgThreadTop.querySelector('.reply-form'))
      msgThreadTop.appendChild(com.composer(thread, { onpost: opts && opts.onpost }))
  }
}

function replies (thread, opts) {
  // collect replies
  var r = []
  ;(thread.related || []).forEach(function(reply) {
    if (reply.value.content.type !== 'vote') { // dont render vote messages, it'd be a mess     
      var el = com.message(reply, opts)
      if (el) {
        r.push(el)
    ...
    var subreplies = replies(reply, opts)
    if (subreplies)
      r.push(subreplies)
  })
