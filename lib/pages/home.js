...
module.exports = function () {
  ...
  function homeStreamWithHelp (opts) {
    // wrap the home stream so that, at the very end, the help renders
    if (opts.lt && opts.lt[0] === 0 && opts.lt[1] === 'help')
      return pull.values([])
    return concat([
      app.ssb.phoenix.createHomeStream(opts),
      pull.values([{ key: 'help', value: { author: 'help', timestamp: 0 }}])
    ])
  }

  function homeRender (msg) {
    if (msg.key == 'help')
      return com.welcomehelp()
    return com.message(msg)
  }
  ...
  ui.setPage('home', h('.layout-twocol',
    h('.layout-main', 
      ...
      com.messageFeed({ render: homeRender, feed: homeStreamWithHelp, filter: homeFilter, infinite: true, live: hlf })),
    h('.layout-rightnav',
      com.sidenav(),
