...
module.exports = function (app) {
  ...
  var imageUploader = com.imageUploader(app, { onupload: onImageUpload, existing: com.profilePicUrl(app, app.user.id) })
  ...
  function post (e) {
    ...
    imageUploader.forceDone(function () {
      ...
      // create the contact
      var contactData = { name: nameInput.value }
      if (profilePic)
        contactData.profilePic = profilePic

      // publish
      app.ui.pleaseWait(true, 500)
      schemas.addContact(app.ssb, app.user.id, contactData, function (err) {
        app.ui.pleaseWait(false)
        if (err) swal('Error While Publishing', err.message, 'error')
        else window.location = '#/profile/'+app.user.id
      })
    })
  }
