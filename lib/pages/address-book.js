...
module.exports = function (app) {
  ...
  app.ssb.gossip.peers(done())
    ...
    function isRecommended (prof) {
      var nfollowers = social.getFollowsByFollowers(app.user.id, prof.id, follows).length
      var nflaggers = social.getFlagsByFollowers(app.user.id, prof.id, follows, trusts).length
    ...
    function renderListing () {
      ...
      if (queryRegex) {
        ...
        // show results
        listing.appendChild(com.contactFeed(app, { filter: searchFilterFn, follows: follows, trusts: trusts }))
      }
      else {
        // show collected lists
        listing.appendChild(h('h3', 'Recommendations'))
        listing.appendChild(com.contactFeed(app, { filter: recommendFilterFn, follows: follows, trusts: trusts }))
        listing.appendChild(h('h3', { style: 'margin-top: 40px' }, 'Currently Following'))
        listing.appendChild(com.contactFeed(app, { filter: friendsFilterFn, follows: follows, trusts: trusts }))
        listing.appendChild(h('h3', { style: 'margin-top: 40px' }, 'Others'))
        listing.appendChild(com.contactFeed(app, { filter: othersFilterFn, follows: follows, trusts: trusts }))
      }
    }
