...
module.exports = function (app) {
  ...
  function setpubstatus () {
    var npubs=0, nactive=0
    app.peers.forEach(function (peer) {
      // :TODO: filter out LAN peers
      if (social.follows(app, peer.key, app.user.id)) {
        npubs++
        if (peer.time && peer.time.connect && peer.time.connect > peer.time.attempt)
          nactive++
      }
    })
    
    var danger1 = (npubs === 0) ? '.text-danger' : ''
    var danger2 = (nactive === 0) ? '.text-danger' : ''
    pubStatusEl.innerHTML = ''
    pubStatusEl.appendChild(h('h3'+danger1, 'You\'re a member of ', npubs,' pubs ', h('small'+danger2, nactive, ' connected')))
    if (npubs === 0)
    ...
    pubStatusEl.appendChild(h('p', h('a.btn.btn-3d', com.icon('cloud'), ' Join a Pub')))
  }
