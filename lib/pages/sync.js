  ...
  ui.setPage('sync', h('.layout-onecol',
    h('.layout-main', 
      o.transform(app.observ.peers, function () {
        ...
        return h('.pub-status',
          h('h3'+danger1, 'You\'re a member of ', stats.membersof,' pubs ', h('small'+danger2, stats.active, ' connected')),
          warning,
          h('p', h('a.btn.btn-3d', { href: '#', onclick: modals.invite }, com.icon('cloud'), ' Join a Pub'))
        )
      }),
      peersEl
    )
  ...
  function renderPeer (peer) {
    ...
    if (peer.time) {
      if (peer.time.connect > peer.time.attempt)
        lastConnect = 'synced '+(new Date(peer.time.connect).toLocaleString())
      else if (peer.time.attempt) {
        lastConnect = 'attempted (but failed) to connect at '+(new Date(peer.time.attempt).toLocaleString())
      }
    }
