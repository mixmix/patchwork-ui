...
var contactPlaque = require('../com/contact-plaque')
var userDownloader = require('../com/user-downloader')
...
module.exports = function () {
    ...
    flaggers.forEach(function (id) {
      ...
      if (flag.reason && flag.key) {
        app.ssb.get(flag.key, function (err, flagMsg) {
          ...
          if (flagMsg) flagMsgs.appendChild(message({ key: flag.key, value: flagMsg }))
        })
      }
    })
  }
  ...
  var hasMsgs = false
  var content = messageFeed({ feed: feedFn, cursor: feedCursor, filter: feedFilter, infinite: true, onempty: onNoMsgs })
  ...
  function onNoMsgs (feedEl) {
    if (hasMsgs) {
      ...
      feedEl.appendChild(h('div', { style: 'margin: 12px 1px; background: #fff; padding: 15px 15px 10px' },
        ...
        h('p', userDownloader(pid))
      ))
    }
  }

  // site link
  var sitelinkEl = h('small')
  app.ssb.patchwork.getSiteLink(pid, function (err, link) {
    if (link) {
      sitelinkEl.appendChild(h('a', { href: '#/webview/'+pid }, 'site'))
    }
  })
  ...
  ui.setPage('profile', h('.layout-twocol',
    ...
    h('.layout-rightnav',
      h('.profile-controls',
        contactPlaque(profile, followers1.length + followers2.length, flaggers.length),
