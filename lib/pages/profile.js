  ...
  app.ssb.friends.all('trust', done())
    ...
    if (nameConflicts.length) {
      ...
      mutualFollowers = (mutualFollowers.length) ?
        h('p', h('strong', 'Who do I follow that follows this user?'), h('ul.list-inline', mutualFollowers)) :
          ...
          [
            ...
            h('.btns',
              h('.btns-group',
                ...
                ' ',
                h('a.btn.btn-3d', { href: '#', onclick: rename }, com.icon('pencil'), ' Rename'),
                ' ',
                h('a.btn.btn-3d', { href: '#', onclick: toggleFlag }, com.icon('flag'), ((isFlagging) ? ' Unflag' : ' Flag'))))
          ] :
