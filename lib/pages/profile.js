...
var views = {
  site: function () {
    require('./webview')({ param: '&BT4kj89QW+yUIa+DpU1wes50SfqRm2DZIN54RQAyPbc=.sha256' })
    // views.profile(true)
  },
  profile: function (nosite) {
    ...
    function feedFilter (msg) {
      ...
      if (msg.value.author == pid && ((c.type == 'post' && !c.root) || c.type == 'init'))
        return true
    }
    ...
    ui.setPage('profile', h('.layout-twocol',
      h('.layout-main',
        (nosite) ?
          h('.well', com.icon('info-sign'), ' This user has not yet published a site.') :
          '',
        h('.profile-header',
          h('h1', h('strong', name)),
...
module.exports = function () {
  ...
  var view = views[app.page.view] || views.site
  view()
