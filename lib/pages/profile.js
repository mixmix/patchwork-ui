...
module.exports = function () {
    ...
    flaggersDlg = h('.profile-flaggers',
      ...
      flaggers.map(function (id) {
        ...
        if (flag.reason) {
          return h('p',
            com.icon('flag'),
            ' "', h('span', { innerHTML: mentions.render(markdown.emojis(flag.reason), flag.mentions) }), '"',
            ' —', com.user(id)
          )
        } else {
          return h('p',
  ...
  ui.setPage('profile', h('.layout-twocol',
    h('.layout-main',
      ...
      com.composer.header({ placeholder: 'Share a public message with @'+name, initval: '@'+name+' ' }),
      content),
    h('.layout-rightnav',
      h('.profile-controls',
        ...
        (!isSelf) ?
          ...
          com.networkGraph({ w: 5.5, drawLabels: false, touchEnabled: false, mouseEnabled: false, mouseWheelEnabled: false }),
  ...
  function privateMessage (e) {
    subwindows.pm(e, { recipients: [pid] })
  }
  ...
  function flagModal (e) {
    ...
    if (!hasFlagged)
      ...
      schemas.addContact(app.ssb, pid, { flagged: false }, function (err) {
        if (err) swal('Error While Publishing', err.message, 'error')
        else ui.refreshPage()
      })
    }
